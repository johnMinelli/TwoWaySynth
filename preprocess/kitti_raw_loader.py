# Preprocessing functions from https://github.com/josephdanielchang/unsupervised_learning_of_depth_pytorch
...
class KittiRawLoader(object):
    ...
    def collect_folders(self):
        ...
        for date in self.date_list:
            drive_set = (self.data_path / date).dirs()
            for dr in drive_set:
                self.scenes.append(dr)

    def collect_from_scene(self, drive):
        train_scenes = []
        for c in self.cam_ids:
            oxts = sorted((drive/'oxts'/'data').files('*.txt'))
            scene_data = {'cid': c, 'dir': drive, 'frame_id': [], 'pose':[], 'rel_path': drive.name + '_' + c}
            scale = None
            origin = None
            ...
            if sample is None:
                return []
            scene_data['P_rect'] = self.get_P_rect(scene_data, zoom_x=sample[1], zoom_y=sample[2])
    ...
    def get_scene_imgs(self, scene_data):
        ...
        for (i,frame_id) in enumerate(scene_data['frame_id']):
            if (self.static_frames is None) or (drive not in self.static_frames.keys()) or (frame_id not in self.static_frames[drive]):
