...
def depth_loss(true, flow_true, pred, flow_pred):
    def im_grad(x):
        x = x.detach().cpu()
        a = torch.Tensor([[1, 0, -1], [2, 0, -2], [1, 0, -1]]).view((1, 1, 3, 3))
        G_x = F.conv2d(x, a)

        b = torch.Tensor([[1, 2, 1], [0, 0, 0], [-1, -2, -1]]).view((1, 1, 3, 3))
        G_y = F.conv2d(x, b)
    ...
    mask = [true>0]  # TODO
    l_depth = torch.mean(torch.abs(pred[mask] - true[mask]), axis=-1)

    # dx_true, dy_true = flow_true[...,0],flow_true[...,1]
    # dx_pred, dy_pred = flow_pred[...,0],flow_pred[...,1]
    dy_true, dx_true = im_grad(true)
    dy_pred, dx_pred = im_grad(pred)
    l_edges = torch.mean(torch.abs(dy_pred - dy_true) + torch.abs(dx_pred - dx_true), axis=-1)

    l_ssim = torch.clip((1 - ssim(true, pred)) * 0.5, 0, 1)
