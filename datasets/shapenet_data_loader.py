...
class ShapeNetDataset(data.Dataset):
    def __init__(self, opt):
        ...
        self.category = "chair"
        self.dataroot = 'D:/datasets/ShapeNet/dataset_%s' % ("chair")

        with open(os.path.join(self.dataroot, 'id_train.txt'), 'r') as fp:
            self.ids_train = [s.strip() for s in fp.readlines() if s]
        # 
        # with open(os.path.join(self.dataroot, 'eval_%s_40.txt' % ("chair")), 'r') as fp:
    ...
    def __getitem__(self, index):
        if self.opt.isTrain:
            ...
            dict = {'A': A, 'B': B, 'RT': mat.astype(np.float32), 'I': np.array([280, 0, 128, 0, 280, 128, 0, 0, 1]).astype(np.float32).reshape((3, 3)), "DB": np.ones((1,256,256))}
            ...
            dict = {'A': A, 'B': B, 'RT': mat.astype(np.float32), 'I': np.array([280, 0, 128, 0, 280, 128, 0, 0, 1]).astype(np.float32).reshape((3, 3)), "DB": np.ones_like(A)}
